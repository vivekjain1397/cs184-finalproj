...
void ParticleManager::init() {
    ...
    //curvature
    // curvature.load("src/shaders/water.vert","src/shaders/curvature.frag");
    ...
    // sending particle radius
    GLint particleRadLoc = glGetUniformLocation(particle_shader.program, "particle_radius"); 
    glUniform1f(particleRadLoc, PARTICLE_RADIUS); 

    // making textures stuff
    glGenTextures(1, &m_dataTexture);
    glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, m_dataTexture);
	glUniform1i(glGetUniformLocation(particle_shader.program, "depthTexture"), 1);

    glGenTextures(1, &m_thicknessTexture);
    glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, m_thicknessTexture);
	glUniform1i(glGetUniformLocation(particle_shader.program, "thicknessTexture"), 2);

    glGenTextures(1, &m_reflectionTexture);
    glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_CUBE_MAP, m_reflectionTexture);
	glUniform1i(glGetUniformLocation(particle_shader.program, "reflectionTexture"), 2);
	...
	glUniform2f( glGetUniformLocation(particle_shader.program, "focalLength"), FOCAL_LENGTH_X, FOCAL_LENGTH_Y );
	// glUniform2f( glGetUniformLocation(curvature.program "texelSize"), texelSizeX, texelSizeY );
	glUniform1f( glGetUniformLocation(particle_shader.program, "timeStep"), 0.001f );
	// glUniform1i( glGetUniformLocation(curvature.program, "depthTexture"), 0 );
